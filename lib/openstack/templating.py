...
from openstack.utils import OPENSTACK_RELEASES
from openstack.context import OSContextGenerator
...
class OSConfigRenderer(object):
    ...
    **Context generators**
    ...
    def register(self, config_file, contexts, config_template=None):
        ...
        logging.info('Registered config file: {}'.format(config_file))
    ...
    def _get_template(self, template):
        ...
        logging.info('Loaded template from {}'.format(template.filename))
        return template
    ...
    def _get_template_from_string(self, ostmpl):
        ...
        logging.info('Loaded a template from a string for {}'.format(
            ostmpl.config_file))
        return template

    def render(self, config_file):
        if config_file not in self.templates:
            logging.error('Config not registered: {}'.format(config_file))
            raise OSConfigException
        ...
        if ostmpl.is_string_template:
            ...
            logging.info('Rendering from a string template: '
                '{}'.format(config_file))
        else:
            ...
            try:
                ...
                try:
                    ...
                    logging.error('Could not load template from {} by {} or {}.'
                        ''.format(
                            self.templates_dir,
            ...
            logging.info('Rendering from template: {}'.format(config_file))
